...
- ❌ **Breakpoint responsive basé uniquement sur `max-width`** — en paysage, un téléphone est **large** (souvent >768px) mais **bas** (<430px) ; sans règle `@media (max-height:...)`, le layout desktop s'applique et rogne le contenu. Toujours prévoir le régime « viewport court » (cf. §10).
- ❌ **`height:100vh` + `overflow:hidden` sur `body` sans échappatoire en viewport court** — piège le contenu sous la ligne de flottaison (boutons/saisie inatteignables en paysage). En viewport court, basculer en `min-height:100dvh` + scroll.
